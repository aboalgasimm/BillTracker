...
export async function GET(request: Request) {
  try {
    ...
    const db = getDb();
    const rows = db.prepare('SELECT * FROM items ORDER BY id DESC').all() as Item[];
...
export async function POST(request: Request) {
  try {
    ...
    // Auto calculate expiration date if not manually overridden
    ...
    const newItem = db.prepare('SELECT * FROM items WHERE id = ?').get(result.lastInsertRowid) as Item;
